...
const isWinregConstructor = (value: unknown): value is WinregConstructor => {
  return typeof value === "function"
}

const loadWinreg = async (): Promise<WinregConstructor> => {
  const module: unknown = await import("winreg")

  if (
    module
    && typeof module === "object"
    && "default" in module
    && isWinregConstructor(module.default)
  ) {
    return module.default
  }

  if (isWinregConstructor(module)) {
    return module
  }

  throw new Error("winreg module did not expose a valid constructor")
}
